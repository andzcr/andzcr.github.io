class ContactManager {
  ...
  setupMessageCounter() {
    ...
    // Inițializează counter-ul la valoarea curentă
    ...
    // Adaugă event listener pentru paste (în caz că user-ul paste)
  ...
  updateMessageCounter() {
    ...
    // Numără caracterele (spațiile se numără ca caractere)
    ...
    // Actualizează și titlul pentru a fi mai informativ
  ...
  setupNameValidation() {
    [this.dom.firstNameInput, this.dom.lastNameInput].forEach(input => {
      input.addEventListener('input', (e) => {
        ...
        // Permite doar litere (fără spații, cifre sau caractere speciale)
        value = value.replace(/[^A-Za-z]/g, '');
        
        // Conversie la prima literă mare, restul mici
        if (value.length > 0) {
  ...
  setupEmailValidation() {
    ...
    this.dom.emailInput.addEventListener('input', (e) => {
      ...
      // Permite litere, cifre, punct, underscore, plus, minus și @
      value = value.replace(/[^a-zA-Z0-9._%+\-@]/g, '');
      
      // Asigură-te că există cel mult un @
